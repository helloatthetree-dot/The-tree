  ...
  return (
    <div className="komorebi-grain min-h-[calc(100vh-4rem)]">
      <div className="max-w-6xl mx-auto px-5 md:px-8 py-14">
        <p className="text-xs uppercase tracking-[0.2em] text-komorebi-clay font-semibold">Café Komorebi</p>
            ...
            <section key={cat} className="mt-12">
              ...
              <div className="mt-6 grid md:grid-cols-2 gap-6">
                {data.items.filter((m) => (m.category || "Other") === cat).map((m, i) => (
                  <motion.div key={m.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (i % 2) * 0.06 }} className="flex gap-4 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline gap-3 border-b border-dashed border-komorebi-border pb-1">
                        <h3 className="font-display text-2xl text-komorebi-ink">{m.name}</h3>
